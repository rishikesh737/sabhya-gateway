...
from app.auth.security import Roles, UserInfo, get_current_user, require_role
from app.config import (  # noqa: F401  # singleton initialized at import for startup readiness
    settings,
...
async def upload_document(
    ...
    try:
        ...
        audit_entry = AuditLog(
            ...
            user_hash=hash_key(current_user.user_id),
            model="rag",
...
async def chat_completion(
    ...
    try:
        all_user_messages = [
            m["content"] for m in chat_req.messages if m["role"] == "user"
        ]
        ...
        if pii_result["pii_detected"] and pii_service.should_block_request(pii_result):
            raise HTTPException(
                status_code=400, detail=pii_service.get_blocking_message(pii_result)
            )
...
# TODO: Implement POST /v1/chat/completions/stream (SSE streaming endpoint)
#   - Required for the "Thinking Process" panel real-time visualization
#   - Should mirror /v1/chat/completions but use StreamingResponse with SSE
#   - Must include audit logging in the finally block (even on failure)
#   - Test stub exists in tests/test_e2e_streaming.py::TestStreamingAuditLog
...
async def list_documents(current_user: UserInfo = Depends(get_current_user)):
    try:
        ...
        if not os.path.exists(data_dir):
            return {"documents": []}
        files = [
            f for f in os.listdir(data_dir) if f.endswith(".pdf") or f.endswith(".txt")
        ]
...
@app.delete(
    "/rag/documents/{filename}", dependencies=[Depends(require_role(Roles.USER))]
)
