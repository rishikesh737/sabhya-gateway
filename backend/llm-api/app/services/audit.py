...
AUDIT_HMAC_SECRET = os.getenv(
    "AUDIT_HMAC_SECRET", "audit-secret-change-in-production-minimum-32-chars"
).encode()
...
class AuditService:
    ...
    def create_entry(
        ...
        logger.debug(
            f"Audit entry created: {log_id[:8]}, hash: {entry.log_hash[:16]}..."
        )
    ...
    def _generate_signature(self, entry: AuditLogEntry) -> str:
        ...
        return hmac.new(
            self.hmac_secret, signature_data.encode(), hashlib.sha256
        ).hexdigest()
    ...
    def verify_integrity(self, entry: AuditLogEntry) -> Dict[str, Any]:
        ...
        if not is_valid:
            logger.warning(f"Audit log integrity check FAILED for {entry.id}: {errors}")
    ...
    def verify_chain(
        self, current: AuditLogEntry, previous: AuditLogEntry
    ) -> Dict[str, Any]:
        """
        ...
        sequence_valid = (
            current.sequence_number is not None
    ...
    def verify_chain_batch(self, entries: List[AuditLogEntry]) -> Dict[str, Any]:
        """
